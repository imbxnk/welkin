<template>
    <v-main>
                ...
                <v-card-text>
                    <v-form class="px-3" v-model="valid" ref="form" lazy-validation>
                        <v-row align='center'>
                            ...
                            <v-col class='d-flex' col='6' md='4' sm='2'>
                                <v-select label="Prefix" :items="newData.prefix" outlined change :rules='prefixRules'
                                    :required="!newData.prefix"></v-select>
                            </v-col>
                            <v-col class='d-flex' col='12' sm='6'>
                                <v-text-field label="First name" v-model='newData.firstname' :rules="firstnameRules"
                                    required></v-text-field>
                            </v-col>
                            <v-col class='d-flex' col='12' sm='6'>
                                <v-text-field label="Last name" v-model="newData.lastname" :rules="lastnameRules"
                                    required></v-text-field>
                            </v-col>
                            <v-col class='d-flex' col='12' sm='6'>
                                <v-text-field label="Nickname" v-model="newData.nickname" :rules="nicknameRules"
                                    required></v-text-field>
                            </v-col>
                            <v-col class='d-flex' col='12' sm='6'>
                                <v-text-field label="Email" v-model="newData.email" :rules="emailRules" required>
                                </v-text-field>
                            </v-col>
                            <v-col class='d-flex' col='6' sm='6'>
                                <v-select v-model="newData.entry_trimester" :items="newData.entry_trimester"
                                    label="Entry Trimester" outlined :rules="entry_trimesterRules" required></v-select>
                            </v-col>
                            <v-col class='d-flex' col='6' sm='6'>
                                <v-text-field label="Entry Year" v-model="newData.entry_year" :rules="entry_yearRules">
                                </v-text-field>
                            </v-col>
        ...
        <template>
            <v-container>
                <v-data-table :headers="headers" :items="tablevalue" :items-per-page="10" class="elevation-1" dark>
                </v-data-table>
                <v-btn color="primary" elevation="2" right outlined>Save</v-btn>
            </v-container>
        </template>
    </v-main>
</template>
    ...
    export default {
        ...
        methods: {
            ...
            addNewStudent: function () {
                ...
                console.log(this.newStudent[1].id, this.newStudent[1].prefix.v, this.newStudent[1].firstname, this
                    .newStudent[1].lastname, this.newStudent[1].nickname, this.newStudent[1].email, this
                    .newStudent[1].entry_trimester, this.newStudent[1].entry_year)
            },
        },
    }
</script>